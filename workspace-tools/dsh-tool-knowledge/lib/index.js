...
const store = {
  ...
  async search(query, limit) {
    ...
    if (q === "") return [];
    const like = "%" + q + "%";
    // 相关度排序：标题精确命中 > 标题以 q 结尾（如「原神wiki/胡桃」）> 标题含 q > 问题含 q > 其它。
    // 起因：FTS5 trigram 至少要 3 个字符，中文人名多为 2 字（胡桃、钟离、甘雨），只能落到 LIKE 分支；
    // 库里只有几十条时 ORDER BY updated_at 够用，但上万条时会把「正文里恰好提到该词的其它页」
    // 排在正主前面（实测 24 个 2 字角色名命中率 0/24）。加入按字段的相关度排序后正主稳定排第一。
    const rankSql = "CASE WHEN title = ? THEN 0 WHEN title LIKE ? THEN 1 WHEN title LIKE ? THEN 2 WHEN problem LIKE ? THEN 3 ELSE 4 END";
    const rankFtsSql = "CASE WHEN fts.title = ? THEN 0 WHEN fts.title LIKE ? THEN 1 WHEN fts.title LIKE ? THEN 2 WHEN fts.problem LIKE ? THEN 3 ELSE 4 END";
    const rankArgs = [q, "%/" + q, like, like];
    if (this.fts && q.length >= 3) {
      try {
        // 修复：fts 虚拟表只有 (id,title,problem,solution,tags) 五列，没有 updated_at。
        // 旧写法 "SELECT ... updated_at FROM fts WHERE fts MATCH ?" 每次都抛
        // "no such column: updated_at"，又被下面的 catch 静默吞掉——FTS 分支其实从未生效过，
        // 所有查询（含 3 字以上）都在走 LIKE 全表扫描。这里 join entries 取回该列，保持返回结构不变。
        // 注意：FTS5 的 MATCH 左操作数必须是表名本身，不能用别名（"f MATCH ?" 会报 no such column: f）。
        return db.prepare("SELECT fts.id AS id, fts.title AS title, fts.tags AS tags, substr(fts.problem,1,240) AS snippet, e.updated_at AS updated_at FROM fts JOIN entries e ON e.id = fts.id WHERE fts MATCH ? ORDER BY " + rankFtsSql + ", fts.rank LIMIT ?").all('"' + q.split('"').join('""') + '"', ...rankArgs, lim);
      } catch (e) { /* 退回 LIKE */ }
    }
    return db.prepare("SELECT id, title, tags, substr(problem,1,240) AS snippet, updated_at FROM entries WHERE title LIKE ? OR problem LIKE ? OR solution LIKE ? OR tags LIKE ? ORDER BY " + rankSql + ", updated_at DESC LIMIT ?").all(like, like, like, like, ...rankArgs, lim);
  },
  async get(id) {
    const db = await this.open();
